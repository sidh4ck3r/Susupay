            ...
            <div className="divide-y divide-white/5">
              {notifications.map((n) => (
                <div 
                  key={n.id} 
                  onClick={() => {
                    console.log('Notification clicked:', n.id);
                    if (n.link) {
                      router.push(n.link);
                    }
                    onClose();
                  }}
                  className="p-4 hover:bg-emerald-500/[0.03] active:bg-emerald-500/[0.05] transition-all cursor-pointer group border-l-2 border-transparent hover:border-emerald-500"
                >
                  <div className="flex gap-3">
                    ...
                    <div className="space-y-1">
                      <p className="text-xs font-bold text-white group-hover:text-emerald-400 transition-colors">{n.title}</p>
