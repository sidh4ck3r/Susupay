    ...
    <div className="min-h-screen px-6 lg:px-10 pb-10 text-slate-100 bg-[#0a0f1a] relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10 mt-10">
        {groups.length > 0 ? groups.map((group: any) => (
          <div key={group.id} className="glass-card p-6 border-white/5 bg-gradient-to-br from-blue-600/5 to-transparent hover:border-blue-500/20 transition-all group">
            ...
            <h4 className="text-lg font-bold text-white mb-2 group-hover:text-blue-400 transition-colors">{group.name}</h4>
            <p className="text-xs text-slate-500 mb-6 line-clamp-2">{group.description}</p>
            
            <div className="flex justify-between items-center text-xs">
              <span className="font-bold text-slate-400 uppercase tracking-widest">Contr: ₵{group.contributionAmount}</span>
              <span className="text-blue-400 font-black px-2 py-1 bg-blue-500/10 rounded-lg">{group.frequency}</span>
            </div>
          </div>
        )) : (
